...
from ionic.ord.gating import safe_exp
...
def I_Na(V: torch.Tensor, m: torch.Tensor,
    ...
    GNa : Maximum conductance (mS/uF)
    nao : Extracellular Na+ (mM)

    Returns
    -------
    INa : Current density (uA/uF)
    """
    ENa = E_Na(nai, nao)
...
def I_NaL(V: torch.Tensor, mL: torch.Tensor,
    ...
    nai : Intracellular Na+ (mM)
    fCaMKp : CaMKII phosphorylation factor
    GNaL : Maximum conductance (mS/uF)
    GNaL_scale : Cell-type scaling factor

    Returns
    -------
    INaL : Current density (uA/uF)
    """
    ENa = E_Na(nai, nao)
...
def I_to(V: torch.Tensor, a: torch.Tensor,
    ...
    fCaMKp : CaMKII phosphorylation factor
    Gto : Maximum conductance (mS/uF)
    Gto_scale : Cell-type scaling factor
    delta_epi : Epicardial fraction (for EPI cells)

    Returns
    -------
    Ito : Current density (uA/uF)
    """
    EK = E_K(ki, ko)
...
def I_CaL(V: torch.Tensor, d: torch.Tensor,
    ...
    Returns
    -------
    ICaL : Ca component (uA/uF)
    ICaNa : Na component (uA/uF)
    ICaK : K component (uA/uF)
    """
...
def I_Kr(V: torch.Tensor, xrf: torch.Tensor, xrs: torch.Tensor,
    ...
    ki : Intracellular K+ (mM)
    GKr : Maximum conductance (mS/uF)
    GKr_scale : Cell-type scaling factor

    Returns
    -------
    IKr : Current density (uA/uF)
    """
    EK = E_K(ki, ko)
...
def I_Ks(V: torch.Tensor, xs1: torch.Tensor, xs2: torch.Tensor,
    ...
    GKs : Maximum conductance (mS/uF)
    GKs_scale : Cell-type scaling factor

    Returns
    -------
    IKs : Current density (uA/uF)
    """
    EKs = E_Ks(ki, nai, ko, nao)
    GKs_eff = GKs * GKs_scale

    # Ca-dependent conductance factor
...
def I_K1(V: torch.Tensor, xk1: torch.Tensor,
    ...
    xk1 : Activation gate
    ki : Intracellular K+ (mM)
    GK1 : Maximum conductance (mS/uF)
    GK1_scale : Cell-type scaling factor

    Returns
    -------
    IK1 : Current density (uA/uF)
    """
    EK = E_K(ki, ko)
...
def I_NaCa(V: torch.Tensor,
    ...
    Returns
    -------
    INaCa_i : Cytosolic component (uA/uF)
    INaCa_ss : Subspace component (uA/uF)
    """
