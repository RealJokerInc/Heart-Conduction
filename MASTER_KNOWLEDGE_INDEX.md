...
| Engine Consolidation | [KNOWLEDGE](Research/Active/engine_consolidation/KNOWLEDGE.md) | Active — **NORTH-STAR (2026-06-01)**: conversational simulation builder for non-coders (unified API + LLM wrapper); build order vocabulary-first → API. 3-engine capability census done (see KNOWLEDGE). **Phase 1 ionic = COPY-ONLY done**: canonical `cardiac_core/ionic/` + editable install + lazy `__init__`; engine rewire + Surrogate/Optimizer migration DEFERRED (audit). **Monodomain V5.5** Cm-correct fork done (exact 1/Cm 3.55e-15; Bidomain cross-check 0.0%/1.1%); V5.4 frozen. Physics note: the Cm time-dilation invariant is FALSE (gates carry no Cm). |
