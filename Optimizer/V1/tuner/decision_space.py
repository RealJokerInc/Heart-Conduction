...
def build_axes(tier: int = 2, *, gNa_floor: Optional[float] = None,
               include_kinetics: bool = True) -> List[Axis]:
    """Assemble the decision axes: conductances (tier) + D_long/D_trans (FREE) +
    optional kinetics. `gNa_floor` widens the g_Na lower bound for the sweep
    (arch lock-2: ≤0.17)."""
    axes: List[Axis] = []
...
def apply(vector, axes: List[Axis], config, *, base_theta: Optional[dict] = None):
    ...
    mesh = chip_mesh(domain_mm=getattr(config, "domain_mm", 16.0),
                     dx_mm=getattr(config, "dx_mm", 0.1),
