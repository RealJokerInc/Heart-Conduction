...
class StateIndex(IntEnum):
    """
    Indices into the state tensor (19 state variables).

    Order follows original TTP06 publication.
    """
    # Membrane potential
    ...
    Cai = 3         # Intracellular Ca2+ cytoplasm (mM)
    CaSR = 4        # SR Ca2+ (mM)
    CaSS = 5        # Subspace Ca2+ (mM)

    # INa gates
    m = 6           # Activation
    h = 7           # Fast inactivation
    j = 8           # Slow inactivation
    ...
    # ICaL gates
    d = 11          # Activation
    f = 12          # Voltage inactivation
    f2 = 13         # Voltage inactivation 2
    fCass = 14      # Ca-dependent inactivation

    # IKr gates
    Xr1 = 15        # Activation
    Xr2 = 16        # Inactivation (time-independent in tissue)

    # IKs gate
    Xs = 17         # Activation

    # RyR (Ryanodine receptor)
    RR = 18         # Release fraction
...
def get_initial_state(device: torch.device = None,
    ...
    """
    Get initial state tensor for TTP06 model.
    ...
    -------
    torch.Tensor
        Initial state tensor of shape (19,)
    """
    if device is None:
