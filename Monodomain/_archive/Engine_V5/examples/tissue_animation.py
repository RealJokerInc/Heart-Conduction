"""
Live Animation of Tissue Wave Propagation
...
class InteractiveSimulation:
    ...
    def apply_stimulus(self):
        ...
        if self.stimulus_active:
            ...
            if elapsed < self.stimulus_duration:
                # Apply negative stimulus current (depolarizing)
                V = self.sim.get_voltage()
                # Direct voltage injection for reliability
                V[self.stim_i_slice, self.stim_j_slice] = np.maximum(
                    V[self.stim_i_slice, self.stim_j_slice],
                    -40.0  # Ensure above threshold
                )
    ...
    def setup_figure(self):
        ...
        self.ax.set_title('Cardiac Wave Propagation (5cm x 5cm)', fontsize=14)
