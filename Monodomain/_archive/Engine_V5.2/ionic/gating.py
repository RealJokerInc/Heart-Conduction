...
def Ito_iFp_tau(V: torch.Tensor) -> torch.Tensor:
    ...
    return Ito_iF_tau(V)  # Same as non-phosphorylated


def Ito_iSp_tau(V: torch.Tensor) -> torch.Tensor:
    """Phosphorylated Ito slow inactivation time constant (ms)."""
    return Ito_iS_tau(V)  # Same as non-phosphorylated


# Ito delta (for EPI and M_CELL)
def Ito_delta_epi(V: torch.Tensor) -> torch.Tensor:
    ...
    Multiplies tiF and tiS for EPI and M_CELL types.
    """
...
def ICaL_fca_inf(cass: torch.Tensor) -> torch.Tensor:
    """
    DEPRECATED: Not used in ORd model.

    The ORd model uses voltage-dependent f_inf (ICaL_f_inf) for fcaf, fcas gates,
    NOT a Ca-dependent steady-state. This function is retained for reference only.
    """
    # Note: This Ca-dependent formulation is NOT used in ORd
    return 1.0 / (1.0 + (cass / 0.00035) ** 1.4)
...
def ICaL_jca_inf(V: torch.Tensor) -> torch.Tensor:
    """
    DEPRECATED: Not used in ORd model.

    The ORd model uses voltage-dependent f_inf (ICaL_f_inf) for the jca gate,
    NOT this formulation. This function is retained for reference only.

    Note: The correct jca_inf is ICaL_f_inf(V), i.e., 1/(1+exp((V+19.58)/3.696))
    """
    # This formula is NOT used in ORd C++ code
    return 1.0 / (1.0 + safe_exp((V + 18.08) / 2.7916))
...
# nca gate (special Ca-dependent factor)
def ICaL_nca_inf(cass: torch.Tensor, km_nca: float = 0.002) -> torch.Tensor:
    """ICaL Ca-dependent factor steady-state."""
    return 1.0 / (km_nca / cass + 1.0)


def ICaL_nca_tau(V: torch.Tensor) -> torch.Tensor:
    """ICaL Ca-dependent factor time constant (ms)."""
    # This is actually handled differently in the model
    return torch.ones_like(V)  # Placeholder
