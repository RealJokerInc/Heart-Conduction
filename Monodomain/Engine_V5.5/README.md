# Engine V5.5: Modular Cardiac Electrophysiology Simulation Engine

> **Fork of V5.4 (2026-05-30).** The ONLY functional change vs V5.4: the operator-split
> reaction (ionic voltage update) divides by the tissue membrane capacitance `Cm`, so
> simulations with `Cm != 1.0` are physically correct (Formulation B, consistent with
> Bidomain V1 / LBM V1). The unused internal LBM path was also removed (it lives in the
> separate `LBM/Engine_V1` engine). **V5.4 remains the frozen validated baseline** — do
> not develop on it. See `Research/Active/engine_consolidation/PLAN.md` for the fork plan.
