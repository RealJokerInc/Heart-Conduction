...
def J_rel(cajsr: torch.Tensor, ICaL: torch.Tensor,
    ...
    ICaL : L-type Ca current (µA/µF) - negative inward current
    ...
    dt : Time step (ms)
    a_rel : Base release amplitude factor (multiplied by bt internally)
    ...
    # Release amplitude: a_rel_np = 0.5 * bt (from ORd C++ code)
    # Jrel_scale is cell-type specific (1.7 for M_CELL)
    a_rel_np = a_rel * Jrel_scale * bt
    # Release is proportional to ICaL (inward current, so use -ICaL)
    Jrel_inf_np = a_rel_np * torch.clamp(-ICaL, min=0.0) / (1.0 + (cajsr_half / cajsr_safe) ** 8)

    tau_rel_np = bt / (1.0 + 0.0123 / cajsr_safe)
    tau_rel_np = torch.clamp(tau_rel_np, min=0.005)  # Minimum 5 µs (from C++)
...
def J_up(cai: torch.Tensor, cansr: torch.Tensor, fCaMKp: torch.Tensor,
    ...
    # From ORd C++: Jupp = 2.75 * 0.004375 * cai / (cai + 0.00092 - 0.00017)
    ...
    # From ORd C++: Jleak = 0.0039375 * cansr / 15.0
...
def buffer_factor_cai(cai: torch.Tensor,
    ...
    The factor is used to scale the effective Ca2+ change:
    d[Ca]_free = buffer_factor * d[Ca]_total
...
def update_concentrations(
    ...
    Parameters
    ----------
    Currents : All membrane currents (µA/µF)
    ...
    # Conversion factor: current (µA/µF) to flux (mM/ms)
    # d[X]/dt = I * Acap / (z * F * volume)
    # where Acap in cm², volume in µL, I in µA/µF
    # This matches V5: dnai = -INa_i * Acap / (F * vmyo)
    cm2_to_uL = Acap / F  # Remove erroneous factor of 1000

    # =========================================================================
    # Sodium
    # =========================================================================
    # Subspace Na+
    # From ORd C++: nass+=dt*(-(ICaNa+3.0*INaCa_ss)*Acap/(F*vss)-JdiffNa)
    INa_ss = ICaNa + 3.0 * INaCa_ss  # Both ICaNa and NCX enter subspace
