...
class SimulationState:
    ...
    # === Voltage (PDE coupling variable) ===
    V: torch.Tensor                  # (n_dof,) transmembrane potential

    # === Ionic States (gates + concentrations only, NO voltage) ===
    ...
    def V_flat(self) -> torch.Tensor:
        return self.V  # Already 1D for classical path
```

**Key design: V is separate from ionic_states.** Voltage is the PDE coupling variable shared between ionic and diffusion solvers. Ionic states (gates + concentrations) are internal to the ionic model. This eliminates the need for `V_index` and avoids dual-storage in LBM.
...
**All solvers access V directly:**

```python
# Uniform access pattern (both classical and LBM):
V = state.V           # Native shape (1D classical, 2D LBM)
V_flat = state.V_flat  # Always 1D (n_dof,)
S = state.ionic_states # Gates + concentrations
```
...
| `V` | `(n_dof,)` | `(n_dof,)` | `(n_dof,)` |
| `ionic_states` | `(n_dof, s)` | `(n_dof, s)` | `(n_dof, s)` |
...
if isinstance(state.spatial, FEMDiscretization):
    ...
    plot_triangles(mesh.nodes, mesh.elements, state.V)
elif isinstance(state.spatial, FDMDiscretization):
    V_grid = state.V.reshape(state.spatial.nx, state.spatial.ny)
    plot_heatmap(V_grid)
```
...
class IonicModel(ABC):
    ...
    V is always passed separately — not stored inside ionic_states.
    """

    # Identity
    name: str
    n_states: int           # Gates + concentrations only (V excluded)
    V_rest: float           # Resting potential (mV)
    gate_indices: List[int]
    concentration_indices: List[int]

    # Initialization
    def get_initial_state(n_cells) -> Tensor: ...  # Returns (n_cells, n_states) WITHOUT V

    # Computation functions (called by IonicSolver)
    # V is always the first parameter, separate from ionic_states
    def compute_Iion(V, ionic_states) -> Tensor: ...
    def compute_gate_steady_states(V, ionic_states) -> Tensor: ...
    def compute_gate_time_constants(V, ionic_states) -> Tensor: ...
    def compute_concentration_rates(V, ionic_states) -> Tensor: ...
```
...
Supports scalar D (uniform) or per-node `D_field=(Dxx, Dxy, Dyy)` tensors of shape (Nx, Ny).
Cardinal stencil uses **harmonic mean** at interfaces: `D_face = 2*D_L*D_R/(D_L+D_R)`.
Cross-term uses center Dxy. When `grid.domain_mask` exists, builds active-index map for
masked assembly (matrix size = n_active x n_active).

```python
# discretization_scheme/fdm.py
class FDMDiscretization(SpatialDiscretization):
    def __init__(self, grid, D=None, D_field=None, chi=1.0, Cm=1.0):
...
Supports scalar D or per-node `D_field=(Dxx, Dyy)` tuple for anisotropic conductivity.
Uses **harmonic mean** at interfaces (TPFA). When `grid.domain_mask` exists, builds
active-index map for masked assembly.
...
class DiffusionSolver(ABC):
    ...
    @abstractmethod
    def step(self, state: SimulationState, dt: float) -> None:
        """Advance diffusion by dt. Modifies state.V in-place."""
        pass
...
class MonodomainSimulation:
    ...
    def __init__(self, spatial, ionic_model, stimulus, dt,
        ...
        V_init = torch.full((spatial.n_dof,), ionic_model.V_rest, device=device, dtype=dtype)
        ionic_states = ionic_model.get_initial_state(n_cells=spatial.n_dof)
...
splitting.step(state, dt):
    ...
    │     │
    │     ├── V = state.V
    │     ├── Iion = self.ionic_model.compute_Iion(V, state.ionic_states)
    │     ├── Istim = self._evaluate_Istim(state)
    │     ├── state.V += (dt/2) * (-Iion + Istim)
    │     ├── gate_inf, gate_tau = model.compute_gate_inf/tau(state.V, state.ionic_states)
...
class RushLarsenSolver(IonicSolver):
    def step(self, state, dt):
        ...
        V = state.V                               # (n_dof,)
        S = state.ionic_states                    # (n_dof, n_ionic_states)

        # 1. Compute ionic current (V separate from ionic_states)
        Iion = model.compute_Iion(V, S)           # (n_dof,)

        # 2. Evaluate stimulus at current time
        Istim = self._evaluate_Istim(state)       # (n_dof,)

        # 3. Forward Euler on voltage
        state.V = V + dt * (-Iion + Istim)

        # 4. Rush-Larsen exponential integration on gates
        V_new = state.V
        gate_inf = model.compute_gate_steady_states(V_new, S)
        gate_tau = model.compute_gate_time_constants(V_new, S)
        ...
        conc_rates = model.compute_concentration_rates(V_new, S)
...
22. Copy `mesh_builder/`, `stim_builder/`, `ui/` from Builder
23. Create `.npz` export pipeline (`mesh_builder/export.py`, `stim_builder/export.py`)
24. Create loaders (`tissue_builder/mesh/loader.py`, `tissue_builder/stimulus/loader.py`)
25. Wire UI `/api/export` endpoint

### Phase 8: Per-Node Conductivity
26. Extend FDM to accept `D_field=(Dxx, Dxy, Dyy)` per-node tensors with harmonic mean at interfaces
27. Extend FVM to accept `D_field=(Dxx, Dyy)` per-node tensors with harmonic mean
28. Add masked assembly (active-index map from `StructuredGrid.domain_mask`)
29. Extend `loader.py` MeshData with `D_xx_grid`, `D_yy_grid`, `D_xy_grid`
...
- [x] `discretization_scheme/fdm.py` - Finite Difference Method (9-pt stencil, Neumann BC)
- [x] `discretization_scheme/fvm.py` - Finite Volume Method (TPFA, harmonic mean)
- [x] Per-node conductivity tensor support (D_field with harmonic mean at interfaces)
- [x] Masked assembly (StructuredGrid.domain_mask → active-index map)

### Temporal Discretization
- [x] `explicit/forward_euler.py` - Forward Euler diffusion
- [x] `explicit/rk2.py` - Heun's method O(dt^2)
- [x] `explicit/rk4.py` - Classical RK4 O(dt^4)
- [x] `implicit/bdf2.py` - 2nd-order BDF

### Linear Solvers
- [x] `linear_solver/chebyshev.py` - GPU-friendly polynomial solver (Gershgorin bounds)
- [ ] `linear_solver/multigrid.py` - Algebraic multigrid (optional, requires pyamgx)
- [x] `linear_solver/fft.py` - DCT (Neumann) / FFT (periodic) for structured grids

### Mesh Types
- [ ] `mesh/tetrahedral.py` - 3D FEM
- [x] `mesh/structured.py` - Structured grids for FDM/FVM/LBM

### Tissue Types
- [x] Per-node D arrays (D_xx, D_yy, D_xy) via Builder export + loader pipeline
- [x] Scar tissue (D=0 → harmonic mean → zero flux)

### LBM
- [x] `lbm/state.py` - LBM state container (distribution functions, streaming, bounce-back)
- [x] `lbm/monodomain.py` - LBM simulation orchestrator with ionic coupling
- [x] `lbm/d2q5.py` - 2D lattice velocity set
- [x] `lbm/d3q7.py` - 3D lattice velocity set
- [x] `lbm/collision.py` - BGK (isotropic) + MRT (anisotropic) operators
- [ ] `lbm/d3q19.py` - 3D high-order velocity set (future)

### Builder Integration
- [x] `mesh_builder/export.py` - SVG → mesh.npz (mask, D fields, label_map)
- [x] `stim_builder/export.py` - SVG → stim.npz (per-region masks + protocols)
- [x] `tissue_builder/mesh/loader.py` - mesh.npz → StructuredGrid + D_field
- [x] `tissue_builder/stimulus/loader.py` - stim.npz → StimulusProtocol
- [x] `ui/server.py` - Flask server with `/api/export` endpoint
- [ ] Voltage clamp stimulus support (direct V setting, not current injection)
