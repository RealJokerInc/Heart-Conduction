...
| 2 | [`02_1_conductances.ipynb`](./02_1_conductances.ipynb) | **Conductances & AP morphology.** A channel-by-channel tour of the six currents that shape the ventricular action potential — `GNa` (upstroke), `Gto` (notch), `PCa` (plateau), `GKr`/`GKs` (repolarisation, the hERG/long-QT story) and `GK1` (resting potential) — each scaled with the one-keyword `conductances={NAME: factor}` knob and overlaid on the baseline. | ~60 s |
| 2A | [`02_2_self_pacing.ipynb`](./02_2_self_pacing.ipynb) | **Self-pacing & the funny current** (advanced). A hiPSC `paci` cell that fires with no electrode (`stim_amplitude=0`): automaticity, diastolic depolarisation, and slowing the spontaneous rate by blocking the funny current `g_f` — the ivabradine mechanism. | ~35 s |
