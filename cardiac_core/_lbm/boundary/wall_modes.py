...
Modes:
  neumann              — generic halfway bounce-back (default; ANY lattice; no overlay)
  hbb                  — the D2Q9 flat-wall HBB baseline; numerically == neumann (no overlay), but it
                         is the specular family's control and is therefore D2Q9-ONLY (see D2Q9_ONLY)
  specular_nextcell    — next-cell specular: flip y, displace 1 cell east/west (D2Q9-only)
  specular_samecell    — flip y, keep x, same cell (== combined, alpha=0) (D2Q9-only)
  combined(alpha)      — HBB (alpha=1) ↔ same-cell specular (alpha=0) blend (D2Q9-only)

All are diagonal→diagonal weight-matched → rest-neutral (no wall pre-charge) and mass-conserving.
Restricted to flat, axis-aligned top/bottom walls; corners + east/west stay HBB. The wall treatment
interacts with β = D·dt/dx² (τ), so compare modes only at matched β.
"""
...
# generic 'neumann' bounce-back applies.
...
def apply_specular_nextcell_d2q9(f: Tensor, f_star: Tensor, NX: int, NY: int) -> Tensor:
    """Next-cell specular reflection: flip y, displace one cell. Non-corner cells only.

    The east and west displacements mirror each other, so the overlay introduces no net
    tangential drift along the wall.
    """
