...
class SolverConvergenceWarning(UserWarning):
    ...
    Emitted (not raised) by default so a silently under-solved diffusion step becomes
    visible instead of propagating a wrong result. Callers who want non-convergence to
    be fatal can escalate it::
...
def warn_nonconvergence(solver, iterations, residual_norm, b_norm, tol, reason="max_iters"):
    ...
    ----------
    solver : str
        Solver name for the message (e.g. ``"PCG"``).
    iterations : int
        Iterations actually performed.
    residual_norm, b_norm : float
        Final residual norm and RHS norm (relative residual = residual_norm / b_norm).
    tol : float
        The relative-residual tolerance that was not met.
    reason : str
        Short cause tag (``"max_iters"``, ``"breakdown"``, ...).
    """
    rel = residual_norm / b_norm if b_norm > 0 else float("inf")
    warnings.warn(
        f"{solver} did not converge ({reason}): {iterations} iters, relative residual "
        f"{rel:.2e} > tol {tol:.1e}. The step result may be inaccurate — increase "
