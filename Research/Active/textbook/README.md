...
> **PDF status:** ✅ working — build with `website/build/html_to_pdf.py` (assembles `website/chapters/*` in `toc.json` order → Playwright → PDF). Current `Cardiac_Computational_Modeling.pdf` = 195 pp, A4. See `/textbook-compile`.
...
- [x] **Part II depth** — BDF2 worked example (Ch 11) + §11.6 expanded (PCG/Chebyshev/FFT + App C.12–14 cross-links) — DONE (Phase 4, commit d5e332a)
- [~] **Part II length** — Ch 8 still ~1100 lines, but added a reading-guide box routing FDM/FEM/FVM/BC readers (Phase 4). Full split deferred.
- [x] **Part IV Ch 18 restructure** — "30-second preview" box added + equations renumbered gapless-ascending + Fig 18.2 — DONE (Phase 4). (§18.1 "Two Cases" trim still open.)
...
- [x] **Part IV Ch 20 depth** — real 5-node pseudo-time worked example (§20.4) + Fig 20.1 dual-lattice — DONE (Phase 4)
...
- [~] **Reader-B accessibility** — improved by the new worked examples + figures (concept layers now stand without the L5 code), but no dedicated parallel non-code path yet. Partially addressed.
- [ ] **Known content gaps** — Ch 4 SVGs → literature images (still SVG); engine-limited topics (Ch 6 chloride, Ch 18 MRT D3Q7, Ch 20 bidomain LBM) documented as "described, not yet in engine".
- [x] **Rebuild the PDF pipeline** — DONE (Phase 5): `website/build/html_to_pdf.py` assembles `website/chapters/*` (toc.json order) → Playwright → PDF. Built `Cardiac_Computational_Modeling.pdf` (195 pp, A4, verified current content).
- [x] **Reconcile tracking-doc claims vs content** — DONE (Phase 5): INDEX corrected — "Quadrature First v4.0" removed (absent from source); "Ω^NR" kept (confirmed present §§19.2–19.4).
