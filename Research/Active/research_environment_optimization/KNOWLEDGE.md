...
### Session Persistence: Our System vs ECC
...
**Our research lifecycle** (with new document architecture):
```
...
Three per-question documents, each with a distinct purpose and resolution level.
...
**Not implementing:**
- **`/aside`** — Use built-in `/btw` instead. See IDEALOG Failed Approaches.
- **WORKLOG.md** — Merged into IDEALOG.md. See IDEALOG Failed Approaches.
...
#### Agent Presentation: Big → Middle → Small (Sequential + Organic)
...
- **plansDirectory UI display**: Dropped. PLAN.md lives in the research question folder, not `./plans/`. The built-in Plan Mode UI is nice-to-have but not load-bearing — we don't depend on it.
- **Adversarial review**: Separate skill (`/review`), opt-in. NOT auto-triggered. Blueprint Phase 4 simply asks: "Plan looks ready. Want adversarial review? (/review)" — user decides.
- **Blueprint vs PROGRESS.md relationship**: Deferred to workflow design phase.
...
| Three-document architecture: KNOWLEDGE + IDEALOG + PLAN | KNOWLEDGE keeps high resolution (facts, analysis, designs, comparisons). IDEALOG is low resolution (condensed narrative). PLAN is for cold-start agent execution. |
| KNOWLEDGE.md keeps high resolution | Detailed comparison tables, design rationale, template formats are all reference material you'd look up later. KNOWLEDGE = everything you'd reference. IDEALOG = the story of how you got there. |
| `/reason` writes on transitions, not every exchange | Accumulate in context, batch-write to IDEALOG.md on: decisions settled, approaches rejected, topic shifts, explicit "write that down", PreCompact. ~3-4 writes per 30min session. Minimizes workflow disruption. |
