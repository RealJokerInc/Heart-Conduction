...
> **✅ EXECUTED 2026-07-23 — all 3 phases shipped in commit `2a4b0e3` on `main`.** 481 passed / 2 xfailed
> (baseline 395/2, +86 tests, **zero regressions**). The implementation gate is closed; this file is now a
> historical record. Deviations + implementation-time findings are in the Mutation Log's final entry.
>
...
- 2026-07-23 — **EXECUTED. All 3 phases implemented, committed (`2a4b0e3`) and pushed to `main`.**
  **Deviation from the plan's per-phase commit points:** the three phases were implemented back-to-back and
  verified by ONE full-suite run rather than three, because the suite takes ~9 min under contention from a
  parallel session in the same working tree. Each phase was still individually test-gated
  (`test_video.py` green after each), and the final gate compared FAILED/ERROR test IDs against a
  pre-implementation baseline: **395 passed / 2 xfailed / 0 failures → 481 passed / 2 xfailed / 0 failures.**
  **Implementation-time findings not anticipated by 6 audit rounds:**
  (1) **`labels=` mutated the caller's `Video` objects** — a render-time override that persisted, and which
  would then make a bare single-clip render of that same object RAISE (label is figure-only). Found by
  self-review, not by any test; fixed to a non-mutating per-panel override + `::test_labels_do_not_mutate_the_caller`.
  (2) `::test_orientation_probe_pixels` failed at `atol=8` purely from **yuv420p chroma subsampling** — H.264 is
  lossy, so a round-tripped pixel drifts ~10 levels at a sharp edge. Tolerance sized to the codec, with a
  companion assertion that opposite corners differ by >60 levels so it still discriminates a flip.
  (3) `render()` needed `labels`/`rows`/`cols` in its OWN signature, not just `_render_panels` — the plan
  specified them only for Phase 2 and the public entry point silently lacked them (`TypeError`).
  (4) A `D=0` scar renders as **non-conducting tissue at rest**, not masked grey — grey is reserved for
  `domain_mask` holes. Confirmed by rendering a real scar simulation and inspecting the frame.
  **Also fixed in the same pass:** the plan's own verification gate had two defects found by measurement —
  plain `-rf` reports ZERO pytest collection errors (so a suite that cannot be collected would PASS the gate;
  now `-rfE`), and `conda run` emits its own `ERROR conda.cli.main_run:...` line that the gate was capturing as
  a bogus test id (now filtered).
