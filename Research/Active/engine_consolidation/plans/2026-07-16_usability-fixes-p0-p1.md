...
## Success Criteria
- [x] All audited bugs in scope (B1–B8, plus B2 wiring) fixed with a regression test each
- [x] The NotImplementedError stub methods no longer advertise a working API (honest errors + de-worked docstrings); `scale_conductance`/`set_conductivity` implemented
- [x] `API_CHEATSHEET.md` corrected (solver/`dt` guidance; drug-block route; `paci`; ORd-LBM-only; `record=`/`save_result`/`dominant_frequency`/`phase_map`; per-node-D; `fiber_angle` radians; bath cost; LBM offset)
- [x] Full suite green (**252 passed / 2 xfailed**; 218 baseline + 34 usability tests) with the new tests added
- [x] Per-engine integrity goldens **bit-identical** (atol=0) at every phase

> **EXECUTED 2026-07-16 on branch `usability-fixes-p0-p1`.** All 5 phases shipped (incl. optional Phase 5).
> Commits: P1 `a37d325`, P2 `d78a86d`, P3 `d94aa6d`, P4 `d6a3237`, P5 `99e1fa3`. Phase 3's diff was
> adversarially audited → 2 cross-engine bugs found + fixed (declarative-bidomain sigma no-op; cell-type flip).
