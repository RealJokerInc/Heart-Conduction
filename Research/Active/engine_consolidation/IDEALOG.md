...
## Current Direction
**Consolidation track — make `cardiac_core` a SINGLE unified ground-up package** (the new PLAN.md, 2026-06-25). The Goal-1 construction API shipped (121 tests, API-track plan archived in `plans/`), but it still *references* the 3 engines from their own folders via the `_prepare_engine()` sys.modules hack. Now: dissolve `cardiac_sim`, vendor the engines IN as a flat unified layout, delete the hack. User decisions (2026-06-25): **Approach A2 — unified ground-up** (not nested vendoring, not rename-only); **copy, don't delete** the engine originals; **unify ionic + mesh + stimulus** as shared top-level packages this pass (tissue/conductivity internals stay per-engine). See the 2026-06-25 thread entry.
...
## Next Step
**Unify cardiac_core into one ground-up package (Approach A2)** — the active plan (PLAN.md, 2026-06-25). Dissolve `cardiac_sim`; vendor mono/bidomain/lbm IN as flat solver packages; extract shared `ionic`(done)+`mesh`(superset)+`stimulus`; rewrite the ~10–20 solver→shared imports to absolute `cardiac_core.*`; delete `_prepare_engine()`; copy-don't-delete the engine originals; 121 tests green + no-cross-folder-reference guard each phase. See the 2026-06-25 thread entry.

Deferred (separate efforts, not this plan): Goal-2 `SimulationSpec`/LLM wrapper; Form-A→B convergence + delete `for_monodomain()`; FEM/TriangularMesh removal; `API_REFERENCE.md` `[design]`→`[now]` tags.
