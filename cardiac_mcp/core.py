...
_PROVENANCE_MARKER = "generated by the cardiac-core MCP server"  # MUST match render_run_script's header
_RUN_FSIZE_BYTES = 4 * 1024**3  # 4 GiB cap on any single file the executed script may write


def _make_run_limits(cpu_seconds: int):
    """POSIX preexec_fn: bound the run_experiment subprocess's CPU time + output file size.

    NO RLIMIT_AS — it caps virtual address space, which CUDA/torch reserve in the tens of GB at
    import, so it would abort torch init even when real memory use is tiny (true memory isolation is
    a container concern; see REMOTE_DEPLOY.md). ``cpu_seconds`` is a loose backstop above the wall
    timeout (multi-threaded torch accrues CPU-time faster than wall-time).
    """
    def _apply() -> None:
        import resource  # POSIX-only

        resource.setrlimit(resource.RLIMIT_CPU, (cpu_seconds, cpu_seconds))
        resource.setrlimit(resource.RLIMIT_FSIZE, (_RUN_FSIZE_BYTES, _RUN_FSIZE_BYTES))
...
def run_experiment(experiment_dir: str, timeout_s: int = 900) -> RunResult:
    ...
    if _PROVENANCE_MARKER not in run_py.read_text():
        raise ValueError(
            "run_experiment(): run.py is not a cardiac-core-generated script; refusing to execute.")
    cpu_budget = timeout_s * (os.cpu_count() or 4)  # CPU-seconds backstop above the wall timeout
    preexec = _make_run_limits(cpu_budget) if platform.system() != "Windows" else None
    proc = subprocess.run(
